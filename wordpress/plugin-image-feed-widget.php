...
Version: 1.0
Author: Yorik van Havre + Caroline Murphy
*/
...
<ul class="image-feed-list">
	<?php // Loop through each feed item and display each item as a hyperlink.
		foreach ( $rss_items as $item ) : 
	?>
	<li>
		<div class="<?php echo $divname; ?>"> <a href="<?php echo $item->get_permalink(); ?>"

					<?php if ($target == 'newwindow') { echo 'rel="external" '; }; ?>

						title="<?php echo str_replace('&', '&amp;', $item->get_title() ); ?>"> <!-- Grabs all the ampersands and makes them friendly HTML -->
					...
					<?php if ($thumb = $item->get_item_tags(SIMPLEPIE_NAMESPACE_MEDIARSS, 'thumbnail') ) {
					   ...
					   /*** Retrieve the filesizes generated by Wordpress Media uploads, and replace with preferred size ***/
					   echo str_replace("540", "100", str_replace("746","135", str_replace("800","100", $images[0][0] ) ) );
...
<p>
	<label for="<?php echo $this->get_field_id('feeds_list__title'); ?>">RSS feeds (comma-separated):
		<input class="widefat" id="<?php echo $this->get_field_id('feeds_list'); ?>" name="<?php echo $this->get_field_name('feeds_list'); ?>" type="text" value="<?php echo attribute_escape($feeds_list); ?>" />
	</label>
</p>
<p>
	<label for="<?php echo $this->get_field_id('maxnumber'); ?>">Max number of images to display:
		<input class="widefat" id="<?php echo $this->get_field_id('maxnumber'); ?>" name="<?php echo $this->get_field_name('maxnumber'); ?>" type="text" value="<?php echo attribute_escape($maxnumber); ?>" />
	</label>
</p>
<p>
	<label for="<?php echo $this->get_field_id('target'); ?>">Where to open the links:
...
<p>
	<label for="<?php echo $this->get_field_id('displaytitle'); ?>">Display title below images?
		<select id="<?php echo $this->get_field_id('displaytitle'); ?>" name="<?php echo $this->get_field_name('displaytitle'); ?>">
			<?php 
...
<p>
	<label for="<?php echo $this->get_field_id('useenclosures'); ?>">Use RSS enclosures? (leave yes if unsure)
		<select id="<?php echo $this->get_field_id('useenclosures'); ?>" name="<?php echo $this->get_field_name('useenclosures'); ?>">
			<?php 
